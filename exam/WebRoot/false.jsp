...
<script type="text/javascript">
	$(function(){
	 ...
	 function newExam(id) {
			location.href = "${pageContext.request.contextPath }/newExam?paper="+id;
	}
	</script>
</head>
<body>
	...
	<div class="zhu">
				...
				<ul>
					<div>
						...
						<c:if test="${exam.issueType==2 }">
							<li class="li"><span>A. </span><input disabled="disabled"
								<c:if test="${fn:contains(exam.answer, 'a')}">
    									checked="checked"
								</c:if>  
								type="checkbox"
								class="true_${status.index+1}" name="xuanZe${status.index }"
								value="a"> <span>${exam.answerA }</span></li>
							<li class="li"><span>B. </span><input disabled="disabled"
								<c:if test="${fn:contains(exam.answer, 'b')}">
    									checked="checked"
								</c:if> 
							 type="checkbox"
								class="true_${status.index+1}" name="xuanZe${status.index }"
								value="b"> <span>${exam.answerB }</span></li>
							<li class="li"><span>C. </span><input  disabled="disabled"
								<c:if test="${fn:contains(exam.answer, 'c')}">
    									checked="checked"
								</c:if> 
							type="checkbox"
								class="true_${status.index+1}" name="xuanZe${status.index }"
								value="c"> <span>${exam.answerC }</span></li>
							<li class="li"><span>D. </span><input disabled="disabled"
								<c:if test="${fn:contains(exam.answer, 'd')}">
    									checked="checked"
								</c:if> 
			...
			<input type="button" id="btnCommit" value="再来一套"  onclick="newExam(${paper.paperId})"/> <input class="btn"
