	...
	$(function() {
		...
		$(".stuName").blur(function() {
			...
			if ($stuName != "") {
				...
				$("#stuName").html("<span class='tiShi'>*用户名不能为空</span>");
				$(".sub").attr({
					"disabled" : "disabled "
				});
				return;
			...
			$.post(
				"${pageContext.request.contextPath}/judge",
				"name="+$name+"&ju=stu",
				function(date){
					if(date.boolean){
...
<body>
	...
	<div class="table">
		...
		<ul>
			...
			<form class="myForm" action="${pageContext.request.contextPath }/stu" method="post">
